...
import { FollowerProfilerDTO } from '@miklebel/watchdog-core'
import {
...
class FollowerProfilerRemover extends Component<IProps, IState> {
  ...
  private async deleteOnClick() {
    await this.props.dispatch(
      deleteFollowerProfilerAsync({ state: this.props.state, props: this.props.followerProfiler })
    )
    this.handleClose()
  ...
  render() {
    return (
      <div>
        ...
        >
          <DialogTitle>{`Delete "${this.props.followerProfiler.name}"?`}</DialogTitle>
          <DialogContent>
...
export default connect(mapStateToProps)(FollowerProfilerRemover)
